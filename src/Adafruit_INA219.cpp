...
void Adafruit_INA219::wireWriteRegister (uint8_t reg, uint16_t value)
{
	...
	if(write(file, buf, length) != length){
		ROS_INFO_STREAM("Error writing to reg");
	}
	/*
...
void Adafruit_INA219::wireReadRegister(uint8_t reg, uint16_t *value)
{
	...
	if(write(file, buf, length) != length){
		ROS_INFO_STREAM("Error writing to reg in read");
	}
	else{
		ROS_INFO_STREAM("Reg written");
	}
	//delay(1);
	...
	if(read(file, data, length) != length){
		ROS_INFO_STREAM("Error reading");
		*value = 0b11111111;
	}
	else{	
		*value = ((buf[0])<<8)|buf[1]; //0b00001111
	}
/*
  Wire.beginTransmission(ina219_i2caddr);
  #if ARDUINO >= 100
...
void Adafruit_INA219::begin(void) {
  // Wire.begin();
	...
	if(ioctl(file, I2C_SLAVE, ina219_i2caddr)<0){
		ROS_INFO("Unable to connect to Device: %d", ina219_i2caddr);
	}
...
int16_t Adafruit_INA219::getCurrent_raw() {
  ...
  // Now we can safely read the CURRENT register!
	wireReadRegister(INA219_REG_CURRENT, &value);
 	ROS_INFO("Current Val: %i", (int16_t)value); 
  return (int16_t)value;
}
